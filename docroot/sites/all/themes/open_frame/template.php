 ...
 */
function open_frame_page_alter(&$page) {
  if (isset($page['content']['system_main']['nodes'])) {
    ...
    if (isset($node['field_blocks']) && $node != -1 && count($node['field_blocks']['#items']) > 0) {
      _open_frame_process_field_blocks($page, $node);
    }
  }
}
 ...
 */
function _open_frame_process_field_blocks(&$page, $node) {
 ...
 */
function open_frame_preprocess_block(&$variables) {
 ...
 */
function open_frame_preprocess_node(&$variables) {
  if ($variables['view_mode'] == 'teaser') {
    if (in_array($variables['type'], array('student_portrait'))) {
      ...
      $grad_year = _open_frame_get_class_year_from_date($variables['node'], 'field_expected_graduation_date');
 ...
 */
function open_frame_js_alter(&$js) {
 ...
 */
function open_frame_breadcrumb($variables) {
...
function open_frame_format_city_state_zip($city, $state, $zip) {
 ...
 *
 */
function open_frame_preprocess_field(&$variables) {
  _open_frame_add_line_breaks($variables);
}
 ...
 * @param $variables
 */
function _open_frame_add_line_breaks(&$variables) {
  if (
...
function _open_frame_get_class_year_from_date($node, $field) {
  $formatted_year = '';
  if (isset($node->{$field})) {
