...
# platform variable:create --level=project --name=LOGS_S3_BUCKET --value=ilrweb-logs --json=false --sensitive=false --prefix=env --visible-build=false --visible-runtime=true
# platform variable:create --level=project --name=LOGS_S3_FOLDER --value=platform-logs --json=false --sensitive=false --prefix=env --visible-build=false --visible-runtime=true
# platform variable:create --level=project --name=LOGS_TMP_PATH --value=/app/tmp --json=false --sensitive=false --prefix=env --visible-build=false --visible-runtime=true

cd $LOGS_TMP_PATH
TODAY_DATE=$(date +%Y-%m-%d)

# This is the name of the log file for a given day.
LOG_FILE=${PLATFORM_PROJECT}-${TODAY_DATE}-access.log
